...
public class WebSocketServer {
	...
	private static final Gson gson = new GsonBuilder().create();
        ...
        private static final ConcurrentHashMap<String, CallMediaPipeline> callPipelines = new ConcurrentHashMap<String, CallMediaPipeline>();
	...
	private void incomingCallResponse(final UserSession callee, JsonObject jsonMessage) throws IOException {
		...
		if ("accept".equals(callResponse)) {
			...
			try {
				...
				callPipelines.put(caller.getSessionId(), pipeline);
				callPipelines.put(callee.getSessionId(), pipeline);
				...
				/*Composite composite = new Composite.Builder(pipeline.getPipeline()).build();
	            ...
	            // take the input from the 2 input feeds
	            /*pipeline.getCalleeWebRtcEp().connect(portIn1);
	            pipeline.getCallerWebRtcEp().connect(portIn2);
	            
	            // Send the composite result to the Callee
	            portOut1.connect(pipeline.getCalleeWebRtcEp());*/
				...
				/*final CallMediaPipeline pl=pipeline;
				new Thread(){
				public void run(){	
					try {
						Thread.sleep(5000);
					...
					pl.getCalleeWebRtcEp().connect(pl.ic2);	
				pl.getCallerWebRtcEp().disconnect(pl.ic2);
...
public void instaCut(UserSession userSession){
	CallMediaPipeline media= callPipelines.get(userSession.getSessionId());
	media.save();
